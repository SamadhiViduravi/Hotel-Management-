...
public class WestminsterHotelManager implements HotelManager {
    ...
    public void addHotelStaff() {
        ...
        if (hotelStaffList.size() < staffLimit) {
            ...
            switch (choiceStaff) {
                ...
                case 3:
                    System.out.println("Enter the years of experience");
                    int yearsOfExperiencee = s.nextInt();
                    System.out.println("Enter the amount of shiftHours");
                    String speciality = s.toString();
                    s.nextLine();

                    Chef chef = new Chef(name, surname);
                    chef.speciality(speciality);
                    chef.yearsOfExperience(yearsOfExperiencee);
    ...
    public void deleteHotelStaff() {
        Scanner s = new Scanner(System.in);
        System.out.println("Enter the staff ID of the member to delete");
        String staffID = s.nextLine();

        for (HotelStaff staffMember : hotelStaffList) {
            if (staffMember.getStaffID().equals(staffID)) {
                System.out.println(staffMember.getName()+" Deleted");
                hotelStaffList.remove(staffMember);

            }
            else{
                System.out.println(staffMember.getName()+" Not Found");
            }
        }
    }
